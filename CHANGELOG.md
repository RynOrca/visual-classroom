...
### Added
- **UnlimitedOCR can now run automatically for scanned PDFs during source ingestion.** The source-processing graph detects PDFs without a usable text layer and, when `UNLIMITED_OCR_COMMAND` is configured, runs the local llama.cpp/UnlimitedOCR helper before content-core and writes the extracted text back into `Source.full_text`. The manual `POST /api/virtual-classroom/ocr` endpoint also runs the OCR subprocess off the event loop (via `asyncio.to_thread`) and returns the correct source id. `.env.example` now documents the llama.cpp server command and the `UNLIMITED_OCR_COMMAND` shape, and `scripts/start-unlimited-ocr-server.sh` starts the local vision server idempotently
- **PDF handling is now modularized** in `open_notebook/virtual_classroom/pdf.py`: file detection, text-layer extraction, and scanned-PDF detection are separated from the OCR command adapter in `ocr.py`, while the source graph continues to use Open Notebook's content-core extraction for normal PDFs and only falls back to UnlimitedOCR for scanned PDFs
- **Virtual classroom knowledge map is now rendered as an interactive React Flow graph.** The previous card list is replaced by a node graph where each chapter/stage is a node, `bridgeToNext` becomes a labeled edge, and the AI storyline is shown as a leading node. Users can pan, zoom, drag nodes, and use the minimap/controls; the `@xyflow/react` dependency was added for this view
- **Conversation organizing agent for virtual classroom.** New `POST /api/virtual-classroom/conversation-notes/organize` reads a chat session's Q&A pairs, asks the configured LLM to classify each exchange into a knowledge card (`definition` / `derivation` / `example` / `pitfall` / `application` / `qa`), extracts tags, and writes `conversation_note` records. It now also runs automatically after source-chat Q&A completes, links to the best-matching existing knowledge point (scored by title/tags/content), and creates a lightweight knowledge point when no good match exists. `GET /api/virtual-classroom/conversation-notes` lists the cards, and the virtual classroom page has a "对话整理" section to browse them
- **Review route generation for virtual classroom.** New `POST /api/virtual-classroom/review/generate` combines the knowledge map, knowledge points, conversation notes and mistakes into a structured “俯瞰 → 下钻” review plan. The result is persisted in the new `review_route` table (migration 26), exposed via `GET /api/virtual-classroom/review`, and rendered in the virtual classroom page as an overview plus per-stage drill-down cards
...
### Fixed
- **Windows test suite is now green.** Podcast path tests use platform-aware path construction/assertions, `api/main.py` is read as UTF-8 instead of the locale encoding, `file://` URI handling in podcast audio paths now supports Windows drive-letter URIs (`file:///C:/...`), and the proxy env-var merge test tolerates case-insensitive Windows environments. Full backend suite: `662 passed`
